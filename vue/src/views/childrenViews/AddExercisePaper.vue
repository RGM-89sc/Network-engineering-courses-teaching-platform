<template>
  <div>
    <el-row v-for="(choiceQuestion, index) in choiceQuestions" :key="choiceQuestion.id">
      <el-card class="question" shadow="hover">
        ...
        <el-input placeholder="请输入题干（必填）" v-model="choiceQuestion.problem">
          <el-select
            style="width: 120px;"
    ...
    <el-button @click="test">ghd</el-button>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.exerciseID = uuid().split('-').join('');
  },
  methods: {
    test() {
      ...
      this.choiceQuestions = this.choiceQuestions.filter(question => {
        if (question.id === id) {
          if (question.detail.imgs.length > 0) {
            // 删除该题目的所有配图
            this.$http
              .post('/api/delQuestionImgs', { imgs: question.detail.imgs }).then(res => {})
              .catch(err => {});
          }
